...
class DistilbertEncoderTransformer(DistilbertEncoderBase):
    ...
    def forward(self, texts: List[str]) -> Union[Tensor, Distribution]:
        # print("texts in distil")
        # print(texts)
        text_encoded_temp, mask_temp = self.get_last_hidden_state(texts, return_mask=True)

        text_encoded = []
        mask = []
        for i in range(int(text_encoded_temp.shape[0]/4)):
            text_encoded_temp_here = (text_encoded_temp[4*i]+text_encoded_temp[4*i+1]+text_encoded_temp[4*i+2]+text_encoded_temp[4*i+3])/4
            mask_temp_here = (mask_temp[4*i]+mask_temp[4*i+1]+mask_temp[4*i+2]+mask_temp[4*i+3])/4

            if(len(text_encoded)==0):
                text_encoded = text_encoded_temp_here.clone().detach()[None]
                mask = mask_temp_here.clone().detach()[None]
            else:
                text_encoded = torch.cat((text_encoded, text_encoded_temp_here[None]), 0)
                mask = torch.cat((mask, mask_temp_here[None]), 0)
