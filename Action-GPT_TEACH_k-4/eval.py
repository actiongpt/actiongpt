        ...
        for keyid in tqdm(dataset._split_index):
            ...
            for index in range(cfg.number_of_samples):
                ...
                try:
                    ...
                    keyid_text = np.load(path / f"{keyid}{seq_id}.npy",
                                           allow_pickle=True).item()['text']                    
                except:
                ...
                if onesample:
                    ...
                    err_root, err_traj = CMetrics.update(model_joints[None], ref_joints[None], [length])
                    error_values[keyid]={"text":keyid_text,"APE_root":err_root.item(),"APE_traj":err_traj.item()}
                    # print(error_values)
                    # with open('samplewise_error_values.json', 'w') as outfile:
                        # outfile.write(json.dumps(error_values))
                        # sys.exit()
                else:
